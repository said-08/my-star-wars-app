...
const Card = ({ locked, name, category, idCard, section, inPage, onCardAction }) => {
  ...
  return (
    <div className={`max-w-sm rounded overflow-hidden shadow-lg 
      ${category === "Especial" ? "bg-[#efb810]" : "bg-blue-500"} 
      ${locked ? "opacity-30 bg-gray-500" : "transform transition-transform duration-300 hover:scale-105 hover:shadow-2xl hover:rotate-1 hover:-translate-y-1"}
        ...
        {!locked && (
          <>
            ...
            <p className="text-gray-700 text-base flex gap-1">
              <span className="font-bold underline">Categoría:</span>
              <div className={`${category === "Especial" ? "bg-[#d3a334]" : 'bg-blue-900'} text-center text-yellow-50 w-[23%] rounded`}>{category}</div>
            </p>
          </>
        )}
      </div>
